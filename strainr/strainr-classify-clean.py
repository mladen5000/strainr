...
from typing import Any, Dict, Generator, List, Optional, Tuple, Union, TextIO # Added TextIO
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("strainr_classify.log"), logging.StreamHandler(sys.stdout)],
)
...
class CliArgs(BaseModel):
    ...
    input_forward: Union[pathlib.Path, List[pathlib.Path]] = Field(
        alias="input_forward", description="Input forward read file(s) (FASTA/FASTQ, possibly gzipped)."
    )
    input_reverse: Optional[Union[pathlib.Path, List[pathlib.Path]]] = Field(
        default=None, alias="input_reverse", description="Optional input reverse read file(s) for paired-end data."
    )
    db_path: pathlib.Path = Field(alias="db", description="Path to the k-mer database file (pickle format).")
    num_processes: int = Field(default=4, ge=1, alias="procs", description="Number of processor cores to use.")
    output_dir: pathlib.Path = Field(default=pathlib.Path("strainr_out"), alias="out", description="Output directory.")
    ...
    abundance_threshold: float = Field(default=0.001, ge=0.0, lt=1.0, alias="thresh", description="Threshold for relative abundance filtering.")
    perform_binning: bool = Field(default=False, alias="bin", description="Perform read binning (if implemented).") # Added specific alias
    save_raw_kmer_hits: bool = Field(default=False, alias="save_raw_hits", description="Save raw k-mer hit counts per read to a CSV file.")
    ...
    def _check_paired_read_consistency(self) -> "CliArgs":
        ...
        if self.input_reverse: # Only if reverse reads are provided
            if isinstance(self.input_forward, list) != isinstance(self.input_reverse, list):
                raise ValueError(
                    "If one of input_forward or input_reverse is a list, the other must also be a list."
                )
            if isinstance(self.input_forward, list) and isinstance(self.input_reverse, list): # Both are lists
...
class KmerClassificationWorkflow:
    ...
    def __init__(self, args: CliArgs) -> None:
        ...
        self.database: Optional[KmerStrainDatabase] = None # Initialized in build_database
    ...
    def _detect_sequence_file_format(self, file_path: pathlib.Path) -> str:
        ...
        with self._open_file_handle(file_path) as f:
            ...
            if first_line.startswith(">"):
                ...
                logger.error(f"Unknown file format for file: {file_path}. First line: '{first_line}'")
    ...
    def _parse_sequence_files(
        ...
        """
        logger.info(f"Parsing sequence files. Forward: {fwd_reads_path}, Reverse: {rev_reads_path if rev_reads_path else 'N/A'}")
        ...
        with self._open_file_handle(fwd_reads_path) as fwd_fh:
            ...
            try:
                ...
                rev_iter: Optional[Any] = None # Iterator for reverse reads, if applicable
                ...
                for fwd_record in fwd_iter:
                    ...
                    if rev_iter:
                        try:
                            ...
                            logger.warning(f"Reverse read file ended before forward file for ID {fwd_id}. Treating remaining as single-end.")
    ...
    def _classify_reads_parallel(
        ...
        sequence_record_iterator = self._parse_sequence_files(fwd_reads_path, rev_reads_path)
    ...
    def _save_raw_kmer_hit_counts(self, raw_hit_results: ReadHitResults, sample_name: str) -> None:
        ...
        if self.database is None:
            logger.error("Cannot save raw hits, database not initialized (needed for strain names).")
            return
        ...
        for read_id, assignment in final_assignments.items():
            if isinstance(assignment, int): # StrainIndex
                if 0 <= assignment < len(self.database.strain_names):
                    ...
                    translated[read_id] = "Unassigned_BadIndex" # Should not happen with proper assignment
    ...
    def _calculate_output_abundances(
        self,
        final_named_assignments: Dict[ReadId, str],
        sample_name: str
    ) -> pd.DataFrame:
        ...
        hit_counts.setdefault("NA", 0) # Assuming "NA" is the marker from ClassificationAnalyzer

        total_assigned_hits = sum(count for strain, count in hit_counts.items() if strain != "NA")
        ...
        for strain_name in self.database.strain_names + ["NA"]:
            ...
            sample_relab = raw_hits / sum(hit_counts.values()) if sum(hit_counts.values()) > 0 else 0.0
            ...
            if strain_name != "NA" and sample_relab >= self.args.abundance_threshold:
                if total_assigned_hits > 0: # Denominator for intra-sample relab
                    ...
                    sum_hits_passing_thresh = sum(
                        h_count for s_name, h_count in hit_counts.items() 
                        if s_name != "NA" and (h_count / sum(hit_counts.values()) if sum(hit_counts.values()) > 0 else 0) >= self.args.abundance_threshold
                    )
            ...
            table_data.append({
                ...
                "intra_relab": intra_relab if strain_name != "NA" else 0.0 # NA doesn't get intra_relab
            })
    ...
    def _display_console_output(self, abundance_df: pd.DataFrame, sample_name: str, top_n: int = 10) -> None:
    ...
    def run_workflow(self) -> None:
        ...
        if self.database is None: # Should be caught by KmerStrainDatabase init if path is bad
        ...
        fwd_files = [self.args.input_forward] if isinstance(self.args.input_forward, pathlib.Path) else self.args.input_forward
        rev_files = []
        if self.args.input_reverse:
            rev_files = [self.args.input_reverse] if isinstance(self.args.input_reverse, pathlib.Path) else self.args.input_reverse
        
        # Ensure output directory exists
        self.args.output_dir.mkdir(parents=True, exist_ok=True)

        for i, fwd_reads_path in enumerate(fwd_files):
            sample_name = fwd_reads_path.name.split('_R1')[0].split('.')[0] # Basic sample naming
            logger.info(f"Processing sample: {sample_name} (File: {fwd_reads_path.name})")
            ...
            raw_kmer_hits: ReadHitResults = self._classify_reads_parallel(fwd_reads_path, rev_reads_path)
            ...
            clear_assignments_idx = analyzer.resolve_clear_hits_to_indices(clear_hits_vec)
            
            prior_counts = analyzer.calculate_strain_prior_from_assignments(clear_assignments_idx)
            prior_prob_vector = analyzer.convert_prior_counts_to_probability_vector(prior_counts)
            
            resolved_ambiguous_idx = analyzer.resolve_ambiguous_hits_parallel(
                ambiguous_hits_vec, prior_prob_vector
            )
            
            final_assignments_idx: Dict[ReadId, Union[StrainIndex, str]] = analyzer.combine_assignments(
                clear_assignments_idx, resolved_ambiguous_idx, no_hit_ids, unassigned_marker="NA"
            )

            # 3. Calculate and output abundances (using internal methods for now)
            final_named_assignments = self._translate_assignments_to_names_for_output(final_assignments_idx)
            abundance_df = self._calculate_output_abundances(final_named_assignments, sample_name)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"A critical error occurred during the StrainR workflow: {e}", exc_info=True)
