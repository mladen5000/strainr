...
class DatabaseBuilder:
    ...
    def _parse_assembly_level(self) -> str:
        ...
        if (
            self.args.assembly_accessions
        ):  # If specific accessions are given, download all levels for them
            return "all"
    ...
    def _download_genomes_from_ncbi(self) -> Tuple[pathlib.Path, pathlib.Path]:
        ...
        if self.args.taxid:
            if (
                self.args.assembly_accessions
            ):  # Ensure mutual exclusivity if not handled by argparse group
                raise ValueError(
            ...
            ncbi_kwargs["assembly_accessions"] = str(
                accessions_file
            )  # ngd expects string path
    ...
    def _filter_genomes_by_unique_taxid(
        ...
        if not metadata_table_path.exists():
            ...
            return list(
                genome_dir.glob("*fna.gz")
            )  # Fallback: use all downloaded .fna.gz files
        ...
        for rel_path_str in filtered_accessions_df["local_filename"]:
            ...
            potential_path = (
                genome_dir / pathlib.Path(rel_path_str).name
            )  # Use only filename part
    ...
    def _get_genome_file_list(
        ...
        if self.args.custom:
            ...
            all_downloaded_files = list(
                genome_download_dir.glob("*fna.gz")
            )  # ngd typically downloads as .fna.gz
    ...
    def _extract_strain_name_from_metadata(
        ...
        if (
            len(accession_match) < 10
        ):  # Heuristic for typical accession like GCF_... or GCA_...
            accession_match = (
                base_name  # Fallback if no underscore or first part is short
            )
        ...
        try:
            if accession_match in metadata_df.index:
                ...
                if pd.isna(strain_info) or strain_info == "nan":
                    strain_info = ""  # Handle actual NaN or "nan" string
