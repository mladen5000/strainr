...
def convert_pickle_to_parquet(pickle_path: Path, parquet_path: Path) -> None:
    ...
    try:
        ...
        print(
            f"Error: The pickle file {pickle_path} is empty or does not contain a valid DataFrame."
        )
    except pickle.UnpicklingError:
        print(
            f"Error: Failed to unpickle data from {pickle_path}. The file may be corrupted or not a pickle file."
        )
