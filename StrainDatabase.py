...
def GCF_to_taxa(df, genomes_folder):
    # for strain names
    ...
    # for taxids
    gcf_to_id = dict(df[["# assembly_accession", "taxid"]].dropna().values)

    # for gcf ids
    gcf_to_id = {k: k for k, v in gcf_to_id.items()}

    fastafiles = glob.glob(os.path.join(genomes_folder, "*fna"))
    print(f"Found {len(fastafiles)} genomes.")
    # as_file = os.path.join(tax_path, 'assembly_summary.txt')
...
def jellyfish_build(kmerlen, GCF_to_tax, GCF_to_filename, genomes_folder):
    ...
    jf_commands = []
    in_out = [
        (GCF_to_filename[k], f"{genomes_folder}/{GCF_to_tax[k]}")
        for k in GCF_to_tax.keys()
...
def main():
    ...
    tax_path = os.path.join(BASEDIR, "taxonomy")
    gdirname = species_tax + "_genomes"
    genomes_folder = os.path.join(BASEDIR, gdirname)
    ...
    # step 4
    gcf_tax_dict, gcf_filename_dict = GCF_to_taxa(df, genomes_folder)
