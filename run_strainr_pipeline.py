...
try:
    from src.strainr.binning import run_binning_pipeline
    # For get_sample_name, if needed directly by runner (classify.py already uses it for output naming)
    from src.strainr.utils import _get_sample_name as get_sample_name
except ImportError:
    ...
    try:
        ...
        print(f"Error: Could not import StrainR modules. Make sure StrainR is installed or src is in PYTHONPATH: {e}")
        sys.exit(1)
...
def main():
    ...
    db_source_group.add_argument("--db_taxid", type=str, help="Species taxonomic ID for NCBI download.")
    db_source_group.add_argument("--db_assembly_accessions", type=str, help="Path to file listing assembly accessions for NCBI.")
    db_source_group.add_argument("--db_genus", type=str, help="Genus name for NCBI download.")
    db_source_group.add_argument("--db_custom_genomes_dir", type=str, help="Path to folder with custom genome FASTA files.")

    db_group.add_argument("--db_kmerlen", type=int, default=31, help="K-mer length for database.")
    db_group.add_argument("--db_name_prefix", type=str, default="strainr_db", help="Output name prefix for the database (e.g., 'my_db').")
    db_group.add_argument("--db_procs", type=int, help="Number of cores for DB building (defaults to --procs if not set).")
    db_group.add_argument("--db_unique_taxid", action="store_true", help="Filter NCBI genomes for unique strain-level taxID.")
    db_group.add_argument("--db_assembly_levels", choices=["complete", "chromosome", "scaffold", "contig"], type=str, default="complete", help="Assembly level for NCBI download.")
    db_group.add_argument("--db_source_ncbi", choices=["refseq", "genbank"], type=str, default="refseq", help="NCBI source for downloads.")


    # --- Classification Arguments ---
    classify_group = parser.add_argument_group("Classification Arguments")
    classify_group.add_argument("--classify_input_forward", nargs='+', required=True, type=str, help="Input forward read file(s) (FASTA/FASTQ).")
    classify_group.add_argument("--classify_input_reverse", nargs='*', type=str, help="Optional input reverse read file(s).")
    classify_group.add_argument("--classify_out_dir", type=str, default="strainr_classification_out", help="Output directory for classification.")
    classify_group.add_argument("--classify_procs", type=int, help="Number of cores for classification (defaults to --procs if not set).")
    classify_group.add_argument("--classify_disambiguation_mode", choices=["random", "max", "multinomial", "dirichlet"], default="max", help="Disambiguation mode.")
    classify_group.add_argument("--classify_abundance_threshold", type=float, default=0.001, help="Abundance threshold.")
    classify_group.add_argument("--classify_save_raw_hits", action="store_true", help="Save raw k-mer hits during classification.")


    # --- Binning Arguments ---
    bin_group = parser.add_argument_group("Binning Arguments")
    bin_group.add_argument("--bin_num_top_strains", type=int, default=2, help="Number of top strains to create bins for.")
    bin_group.add_argument("--bin_out_dir", type=str, help="Output directory for binned FASTQ files (defaults to a 'bins' subdir in --classify_out_dir).")
    bin_group.add_argument("--skip_binning", action="store_true", help="Skip the binning step.")

    # --- General Arguments ---
    general_group = parser.add_argument_group("General Arguments")
    general_group.add_argument("--procs", type=int, default=4, help="Default number of cores for all steps (can be overridden).")
    general_group.add_argument("--verbose", action="store_true", help="Enable verbose output for sub-scripts.")
    ...
    classify_procs = args.classify_procs if args.classify_procs is not None else args.procs

    # --- Step 1: Build Database ---
    logger.info("--- Starting Step 1: Database Building ---")
    db_command = ["python", "src/strainr/build_db.py",
                  "--kmerlen", str(args.db_kmerlen),
                  "--out", args.db_name_prefix,
                  "--procs", str(db_procs),
                  "--assembly_levels", args.db_assembly_levels,
                  "--source", args.db_source_ncbi
    ]
    if args.db_taxid:
    ...
    base_classify_command = ["python", "src/strainr/classify.py",
                       "--db", str(db_output_path),
                       "--out", str(classify_out_dir_path),
                       "--procs", str(classify_procs),
                       "--mode", args.classify_disambiguation_mode,
                       "--thresh", str(args.classify_abundance_threshold)
    ]
    ...
    num_rev_files = len(args.classify_input_reverse) if args.classify_input_reverse else 0

    if num_rev_files > 0 and num_fwd_files != num_rev_files:
        logger.error("Mismatch in number of forward and reverse read files for classification.")
        sys.exit(1)
    ...
    for i in range(num_fwd_files):
        ...
        logger.info(f"Classifying sample: {sample_name_for_output} (from {fwd_read_file})")
        ...
        run_command(current_classify_command, f"Classification for {sample_name_for_output}")

        # Store info for binning
        assignments_file = classify_out_dir_path / f"{sample_name_for_output}_final_assignments.pkl"
        strains_file = classify_out_dir_path / f"{sample_name_for_output}_strain_names.txt"
        ...
        if assignments_file.exists() and strains_file.exists():
            processed_samples_data.append({
                ...
                "rev_reads": rev_read_file if (args.classify_input_reverse and i < num_rev_files) else None,
                "assignments_pkl": assignments_file,
                "strain_names_txt": strains_file,
                "classify_out_dir": classify_out_dir_path
            })
        else:
            logger.warning(f"Output files for binning not found for sample {sample_name_for_output}. Skipping binning for this sample.")
    ...
    if args.skip_binning:
        ...
        logger.info("--- Skipping Step 3: Binning (no samples successfully classified for binning inputs) ---")
    else:
        ...
        for sample_data in processed_samples_data:
            ...
            try:
                ...
                if args.bin_out_dir:
                    ...
                    effective_bin_parent_dir = sample_data["classify_out_dir"] # This is already a pathlib.Path
                ...
                run_binning_pipeline(
                    ...
                    reverse_reads_fastq=pathlib.Path(sample_data["rev_reads"]) if sample_data["rev_reads"] else None
                )
                final_bin_location = effective_bin_parent_dir / "bins"
                logger.info(f"Binning completed for sample: {sample_data['sample_name']}. Output in {final_bin_location}")

            except Exception as e:
                logger.error(f"Error during binning for sample {sample_data['sample_name']}: {e}")
