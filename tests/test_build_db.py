...
def create_gzipped_fasta_file(content: str, output_path: pathlib.Path):
    ...
    with (
        open(uncompressed_path, "rb") as f_in,
        gzip.open(output_path, "wb") as f_gz_out,
    ):
...
class TestDatabaseBuilder(unittest.TestCase):
    ...
    def test_parse_assembly_level_complete(self):
        ...
        self.builder.args.assembly_accessions = (
            None  # Ensure this is None for this test case
        )
    ...
    def test_parse_assembly_level_scaffold(self):
        ...
        self.builder.args.assembly_accessions = None
        self.assertEqual(
            self.builder._parse_assembly_level(), "complete,chromosome,scaffold"
        )
    ...
    def test_parse_assembly_level_contig(self):
        ...
        self.builder.args.assembly_accessions = None
        self.assertEqual(
            self.builder._parse_assembly_level(), "complete,chromosome,scaffold,contig"
        )

    def test_parse_assembly_level_with_accessions(self):
        self.builder.args.assembly_accessions = (
            "some_file.txt"  # Value doesn't matter, just its presence
        )
    ...
    def test_extract_strain_name_no_metadata(self):
        ...
        type(mock_genome_file).name = PropertyMock(
            return_value="GCF_000123.1_ASM123v1.fna.gz"
        )
    ...
    def test_extract_strain_name_empty_metadata_df(self):
        ...
        name = self.builder._extract_strain_name_from_metadata(
            mock_genome_file, empty_df
        )
        self.assertEqual(name, "GCF_000123.1_ASM123v1")
    ...
    def test_extract_strain_name_metadata_found_full_info(self):
        ...
        metadata_dict = {
            "assembly_accession": [
                "GCF_000123.1"
            ],  # Note: GCF_000123.1 derived from filename
    ...
    def test_extract_strain_name_metadata_found_no_strain_info(self):
        mock_genome_file = MagicMock(spec=pathlib.Path)
        type(mock_genome_file).name = PropertyMock(
            return_value="GCF_000456.1_ASM456v1.fna.gz"
        )
    ...
    def test_extract_strain_name_metadata_found_nan_strain_info(self):
        mock_genome_file = MagicMock(spec=pathlib.Path)
        type(mock_genome_file).name = PropertyMock(
            return_value="GCF_000789.1_ASM789v1.fna.gz"
        )
    ...
    def test_extract_strain_name_accession_not_in_metadata(self):
        mock_genome_file = MagicMock(spec=pathlib.Path)
        type(mock_genome_file).name = PropertyMock(
            return_value="GCF_000999.1_ASM999v1.fna.gz"
        )
...
"""  # GCA_004 has missing species_taxid, still considered unique if taxid present
        ...
        (
            genome_dir / "GCA_004.fna.gz"
        ).touch()  # Kept: taxid present, species_taxid is NaN effectively
        ...
        def mock_path_exists_filter(path_obj):
            ...
            if path_obj.name == f"filtered_{mock_metadata_path.name}":
                return True  # For saving filtered metadata
            return False
        ...
        with (
            ...
            filtered_files = self.builder._filter_genomes_by_unique_taxid(
...
"""  # Missing local_filename column
        ...
        )  # Should return empty if no local_filename
...
"""  # local_filename points to a file that won't exist
    ...
    def test_process_single_fasta_for_kmers_writes_correct_kmers(
        ...
        a_string, written_kmer_count, out_path = (
            self.builder._process_single_fasta_for_kmers(
                genome_file_info,
        ...
        self.assertEqual(
            out_path, temp_kmer_file_path
        )  # Ensure it returns the correct path
    ...
    def test_create_database_custom_genomes(self):
        ...
        self.assertTrue(
            ...
            f"Database Parquet file not found at {expected_db_path}",
        )
        ...
        db_kmers_set = set(
            kmer.decode("utf-8") if isinstance(kmer, bytes) else kmer
            for kmer in db_df.index
        )
        ...
        col_strain_a = (
            "strainA" if "strainA" in db_df.columns else "strainA_1"
        )  # Adjust if names get numbered
        col_strain_b = "strainB" if "strainB" in db_df.columns else "strainB_1"
        if (
            "strainA" not in db_df.columns and "strainA_1" in db_df.columns
        ):  # if numbering happens
            ...
            if col_strain_a not in db_df.columns:
                col_strain_a = db_df.columns[0]  # Fallback if names are unexpected
            if col_strain_b not in db_df.columns:
                col_strain_b = db_df.columns[1]
    ...
    def test_create_database_ncbi_mocked(self, mock_ngd_download):
        ...
        simulated_ncbi_download_dir = (
            self.builder.output_db_dir / f"ncbi_genomes_{genome_target_dir_suffix}"
        )
        ...
        col_strain1, col_strain2 = (
            db_df.columns[0],
            db_df.columns[1],
        )  # Assume order for simplicity for now
        ...
        if kmer_gcg_bytes in db_df.index:
            ...
            self.fail(
                f"Kmer {kmer_gcg_bytes.decode()} or {kmer_atg_bytes.decode()} not found in db index."
            )
