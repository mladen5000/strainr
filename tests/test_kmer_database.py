...
def pickled_kdb_path(  # Renamed
    ...
    if params.get("empty_df", False):
        ...
        if kmer_type == "str":
            kmers_for_df = [
                sample_kmers_str_kdb[0],
                sample_kmers_str_kdb[1][: default_kmer_length_kdb - 1],
            ]
        else:  # bytes
            kmers_for_df = [
                sample_kmers_bytes_kdb[0],
                sample_kmers_bytes_kdb[1][: default_kmer_length_kdb - 1],
            ]
        ...
        counts_data = np.array(
            [["str_val1", "str_val2"], ["str_val3", "str_val4"]], dtype=object
        )  # type: ignore
        df_to_pickle = create_dummy_dataframe_for_kdb(
            kmers_for_df[:2], strains, counts_data
        )  # Match dimensions
    elif params.get("non_unique_kmers", False):
        kmers_for_df = [kmers_for_df[0], kmers_for_df[0]] + kmers_for_df[
            1:
        ]  # Duplicate first kmer
        df_to_pickle = create_dummy_dataframe_for_kdb(kmers_for_df, strains)
...
def test_kdb_init_success_str_kmers(
    ...
    db = StrainKmerDb(pickled_kdb_path, expected_kmer_length=default_kmer_length_kdb)
...
@pytest.mark.parametrize("pickled_kdb_path", [{"kmer_type": "str"}], indirect=True)
def test_kdb_init_kmer_length_inferred(
    pickled_kdb_path: pathlib.Path,
    default_kmer_length_kdb: int,
    capsys: pytest.CaptureFixture,
):
    db = StrainKmerDb(pickled_kdb_path, expected_kmer_length=None)  # Infer length
    assert db.kmer_length == default_kmer_length_kdb
...
@pytest.mark.parametrize("pickled_kdb_path", [{"kmer_type": "str"}], indirect=True)
def test_kdb_init_kmer_length_mismatch_error(
    pickled_kdb_path: pathlib.Path, default_kmer_length_kdb: int
):
    with pytest.raises(ValueError, match="does not match length of first k-mer"):
        StrainKmerDb(pickled_kdb_path, expected_kmer_length=default_kmer_length_kdb + 1)
...
@patch("pandas.read_pickle")
def test_kdb_init_pickle_not_dataframe_error(
    mock_read_pickle: MagicMock, tmp_path: pathlib.Path
):
...
@pytest.mark.parametrize(
    "pickled_kdb_path", [{"inconsistent_kmer_len": True}], indirect=True
)
def test_kdb_init_inconsistent_kmer_length_error(
    pickled_kdb_path: pathlib.Path, default_kmer_length_kdb: int
):
    with pytest.raises(
        ValueError,
        match=f"Inconsistent k-mer string length at index 1. Expected {default_kmer_length_kdb}",
    ):
        StrainKmerDb(pickled_kdb_path, expected_kmer_length=None)  # Infer length


@pytest.mark.parametrize(
    "pickled_kdb_path", [{"unsupported_kmer_type": True}], indirect=True
)
...
@pytest.mark.parametrize(
    "pickled_kdb_path",
    [{"custom_kmers": ["AA", "ÄA"], "kmer_type": "str"}],
    indirect=True,
)
def test_kdb_init_inconsistent_byte_len_after_encoding_error(
    pickled_kdb_path: pathlib.Path, capsys
):
    ...
    with pytest.raises(
        ValueError, match="Post-encoding/cast k-mer byte length validation failed"
    ):
        StrainKmerDb(pickled_kdb_path, expected_kmer_length=2)  # Expect string length 2


@pytest.mark.parametrize(
    "pickled_kdb_path", [{"non_numeric_counts": True}], indirect=True
)
...
@pytest.mark.parametrize(
    "pickled_kdb_path", [{"non_unique_kmers": True}], indirect=True
)
def test_kdb_init_non_unique_kmers_warning(
    pickled_kdb_path: pathlib.Path, capsys: pytest.CaptureFixture
):
    StrainKmerDb(pickled_kdb_path)  # Should load, using last occurrence for duplicates
    captured = capsys.readouterr()
...
@pytest.mark.parametrize(
    "pickled_kdb_path", [{"kmer_type": "str", "valid_data": True}], indirect=True
)
...
def test_kdb_len_method(
    pickled_kdb_path: pathlib.Path, sample_kmers_str_kdb: List[str]
):
    db = StrainKmerDb(pickled_kdb_path)
    ...
    [
        {
            ...
            "custom_kmers": ["AAAA", "AAAA", "CCCC", "GGGG"],
            "kmer_type": "str",
        }
    ],
    indirect=True,
)
def test_kdb_len_method_non_unique_kmers(
    pickled_kdb_path: pathlib.Path, default_kmer_length_kdb: int
):
    ...
    db = StrainKmerDb(pickled_kdb_path, expected_kmer_length=4)
    assert len(db) == 3
