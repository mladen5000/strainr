...
import pathlib
import pickle
from typing import List, Dict, Union, Any, Optional, Any  # Added Optional
...
def create_dummy_dataframe_for_db(
    ...
    if not strain_names:
        return pd.DataFrame(
            index=pd.Index(kmer_list, name="kmer_index")
...
@pytest.fixture
def strain_names_fixture_db() -> List[
    str
]:  # Renamed to avoid conflict if other test files use similar names
...
def pickled_db_file_db(  # Renamed to avoid conflict
    ...
    no_kmers_df = params.get(
        "no_kmers_df", False
    )  # DataFrame with columns but no kmer rows
    no_strains_df = params.get(
        "no_strains_df", False
    )  # DataFrame with kmer rows but no columns
...
def test_db_init_empty_pickle_file_error(
...
def test_db_init_inconsistent_kmer_lengths_error(
    pickled_db_file_db: pathlib.Path, default_kmer_length_db: int
):
...
def test_db_lookup_kmer_found_and_not_found(
    pickled_db_file_db: pathlib.Path, strain_names_fixture_db: List[str]
):
    db = StrainKmerDatabase(
        pickled_db_file_db, kmer_length=5
    )  # kmer_length=5 from data
...
def test_db_get_database_stats(
    ...
    assert (
        stats["strain_names_preview"] == strain_names_fixture_db[:5]
    )  # Assuming at least 3-5 strains for preview
