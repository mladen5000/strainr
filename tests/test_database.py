...
def parquet_db_file_db(
    ...
    if kmer_type == "str":
        ...
        default_kmers = params.get("kmer_data", [12345]) # Default to a single int if not provided
    else: # bytes
        default_kmers = [
...
@pytest.mark.parametrize(
    "parquet_db_file_db", [{"kmer_type": "str"}], indirect=True
)
def test_db_init_successful_kmers_as_str(
    ...
    db = StrainKmerDatabase(
        parquet_db_file_db, expected_kmer_length=default_kmer_length_db # Corrected
    )
    ...
    assert all(isinstance(k, bytes) for k in db.kmer_to_counts_map.keys()) # Corrected attribute
    assert db.strain_names == strain_names_fixture_db


@pytest.mark.parametrize(
    "parquet_db_file_db", [{"kmer_type": "bytes"}], indirect=True
)
...
def test_db_init_kmer_length_mismatch_error( # Renamed test, checking for error now
    ...
    with pytest.raises(ValueError, match=f"Provided expected_kmer_length \\({constructor_kmer_len}\\) does not match "
                                       f"length of first k-mer in database \\({default_kmer_length_db}\\)."):
        StrainKmerDatabase(
            parquet_db_file_db, expected_kmer_length=constructor_kmer_len # Corrected
        )
...
def test_db_init_file_not_found():
    with pytest.raises(
        ...
        StrainKmerDatabase(
            "non_existent_file_for_db.parquet", expected_kmer_length=5 # Corrected
        )
...
def test_db_init_empty_or_corrupt_parquet_file_error(
    ...
    with pytest.raises(
        ...
        StrainKmerDatabase(db_path, expected_kmer_length=5) # Corrected
...
@pytest.mark.parametrize(
    "parquet_db_file_db", [{"empty_df": True}], indirect=True
)
def test_db_init_empty_dataframe_value_error(
    parquet_db_file_db: pathlib.Path,
):
    with pytest.raises(ValueError, match=r"Loaded database is empty: .*"): # Corrected regex
        StrainKmerDatabase(parquet_db_file_db, expected_kmer_length=5) # Corrected


@pytest.mark.parametrize(
    "parquet_db_file_db", [{"no_kmers_df": True}], indirect=True
)
...
@pytest.mark.parametrize(
    "parquet_db_file_db", [{"no_strains_df": True}], indirect=True
)
...
def test_db_init_inconsistent_kmer_lengths_warning( # Renamed
    parquet_db_file_db: pathlib.Path, default_kmer_length_db: int, capsys: pytest.CaptureFixture
):
    db = StrainKmerDatabase(
        parquet_db_file_db, expected_kmer_length=None # Allow inference
    )
    captured = capsys.readouterr()
    assert f"Warning: Inconsistent k-mer string length at index 1. Expected {KMER_LEN_FOR_TESTS}" in captured.out
    assert "Skipping." in captured.out
    assert db.num_kmers == 1 # Only the first k-mer should be loaded
...
def test_db_init_non_numeric_data_in_df_error(
    ...
    with pytest.raises(
        TypeError, match="Could not convert database values to count matrix" # Corrected to TypeError
    ):
        StrainKmerDatabase(
            parquet_db_file_db, expected_kmer_length=default_kmer_length_db # Corrected
        )
...
def test_db_get_strain_counts_for_kmer_found_and_not_found( # Renamed
    ...
    db = StrainKmerDatabase(parquet_db_file_db, expected_kmer_length=5) # Corrected

    known_kmer_str = "ATGCG"
    known_kmer_bytes = known_kmer_str.encode("utf-8")
    count_vector = db.get_strain_counts_for_kmer(known_kmer_bytes) # Corrected method name
    ...
    assert db.get_strain_counts_for_kmer(unknown_kmer_bytes) is None # Corrected method name

    incorrect_length_kmer = ("ATGC").encode("utf-8")
    assert db.get_strain_counts_for_kmer(incorrect_length_kmer) is None # Corrected method name
