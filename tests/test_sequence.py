...
from src.strainr.sequence import GenomicSequence, extract_kmers_from_sequence, VALID_DNA_BYTES
...
def test_genomic_sequence_creation_success(genomic_sequence_fixture: GenomicSequence, valid_sequence_id_fixture: str, valid_dna_bytes_fixture: bytes):
...
def test_genomic_sequence_invalid_dna_chars_error(valid_sequence_id_fixture: str):
    with pytest.raises(ValueError, match=r"Sequence contains invalid DNA bytes: \{'X' \(byte: 88\)\}. Allowed bytes correspond to characters: 'A', 'C', 'G', 'N', 'T'."): # Corrected regex
        GenomicSequence(
            sequence_id=valid_sequence_id_fixture, sequence_data=b"ACGTX"
        )  # X is invalid


def test_genomic_sequence_lowercase_conversion_and_validation(valid_sequence_id_fixture: str):
    # The current _validate_sequence_data is case-sensitive and only allows uppercase ACGTN.
    # If lowercase were to be accepted and converted, this test would change.
    # As it stands, lowercase 'a' (byte 97) is invalid.
    with pytest.raises(ValueError, match=r"Sequence contains invalid DNA bytes: \{'a' \(byte: 97\)\}. Allowed bytes correspond to characters: 'A', 'C', 'G', 'N', 'T'."):
...
def test_genomic_sequence_non_ascii_decode_error(valid_sequence_id_fixture: str):
    ...
    with pytest.raises(ValueError, match=r"Sequence contains invalid DNA bytes: \{byte 164, byte 195\}. Allowed bytes correspond to characters: 'A', 'C', 'G', 'N', 'T'."): # Corrected regex
        GenomicSequence(
            sequence_id=valid_sequence_id_fixture, sequence_data=b"ACGT\xc3\xa4N"
        )  # 'ä' in UTF-8


def test_genomic_sequence_length(genomic_sequence_fixture: GenomicSequence, valid_dna_bytes_fixture: bytes):
...
def test_genomic_sequence_hash(
    ...
    seq4 = GenomicSequence(
        sequence_id=valid_sequence_id_fixture, sequence_data=b"ACGTACGT" # Different sequence
    )

    assert isinstance(hash(seq1), int) # This should pass if mmh3.hash_bytes returns an int
    assert hash(seq1) == hash(seq2)  # Same data, same hash
    assert hash(seq1) == hash(seq3)  # Same data (sequence_id not part of hash by default for frozen dataclass unless explicitly included)
...
def test_genomic_sequence_is_valid_dna(genomic_sequence_fixture: GenomicSequence):
    assert genomic_sequence_fixture.is_valid_dna() is True # Should always be true for a successfully constructed object
...
def test_extract_kmers_kmer_length_positive_error():
    ...
    with pytest.raises(ValueError, match=r"kmer_length must be positive, got 0\."): # Added \. for literal period
...
def test_extract_kmers_invalid_sequence_type_error():
    with pytest.raises(
        ...
        extract_kmers_from_sequence("not_a_genomic_sequence", kmer_length=3) # type: ignore
