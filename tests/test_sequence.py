...
def test_genomic_sequence_invalid_dna_chars_error(valid_sequence_id_fixture: str):
    with pytest.raises(ValueError, match=r"Sequence contains invalid DNA bytes: \{'X' \(byte: 88\)\}. Allowed bytes correspond to characters: 'A', 'C', 'G', 'N', 'T'."):
        GenomicSequence(
...
def test_genomic_sequence_lowercase_conversion_and_validation(valid_sequence_id_fixture: str):
    with pytest.raises(ValueError, match=r"Sequence contains invalid DNA bytes: \{'a' \(byte: 97\), 'c' \(byte: 99\), 'g' \(byte: 103\), 't' \(byte: 116\)\}. Allowed bytes correspond to characters: 'A', 'C', 'G', 'N', 'T'."):
...
def test_genomic_sequence_non_ascii_decode_error(valid_sequence_id_fixture: str):
    with pytest.raises(ValueError, match=r"Sequence contains invalid DNA bytes: \{byte 164, byte 195\}. Allowed bytes correspond to characters: 'A', 'C', 'G', 'N', 'T'."):
        GenomicSequence(
...
def test_genomic_sequence_is_valid_dna(genomic_sequence_fixture: GenomicSequence):
    assert genomic_sequence_fixture.is_valid_dna() is True
