...
def calculator_fixture(
    strain_names_fixture: List[str], default_threshold_fixture: float
) -> AbundanceCalculator:
    """Provides a default AbundanceCalculator instance."""
    return AbundanceCalculator(
        strain_names=strain_names_fixture, abundance_threshold=default_threshold_fixture
    )
...
def test_convert_assignments_indices_to_names(
    calculator_fixture: AbundanceCalculator, strain_names_fixture: List[str]
):
    ...
    assert (
        calculator_fixture.convert_assignments_to_strain_names(read_assignments)
        == expected
    )
...
def test_convert_assignments_mixed_indices_and_names(
...
def test_convert_assignments_non_str_read_ids_converted(
    calculator_fixture: AbundanceCalculator, strain_names_fixture: List[str]
):
    read_assignments: Dict[Union[str, int], Union[str, int]] = {
    ...
    expected = {
        "1001": strain_names_fixture[0],
        "read_str_002": strain_names_fixture[1],
    }
    assert (
        calculator_fixture.convert_assignments_to_strain_names(read_assignments)
        == expected
...
def test_calculate_raw_abundances_simple_counts(
    calculator_fixture: AbundanceCalculator,
):
    named_assignments = {
        "r1": "StrainA",
...
def test_calculate_raw_abundances_exclude_unassigned_true_default(
    ...
    assert (
        calculator_fixture.calculate_raw_abundances(
            named_assignments, unassigned_marker=unassigned_marker_fixture
        )
        == expected
    )
...
def test_calculate_relative_abundances_empty_input(
    calculator_fixture: AbundanceCalculator,
):
    assert calculator_fixture.calculate_relative_abundances(Counter()) == {}


def test_calculate_relative_abundances_all_zero_counts(
    calculator_fixture: AbundanceCalculator,
):
...
def test_apply_threshold_and_format_filtering_and_sorting(
    ...
    result_abundance = calculator_fixture.apply_threshold_and_format(
        relative_abundances, sort_by_abundance=True
    )
    ...
    result_name = calculator_fixture.apply_threshold_and_format(
        relative_abundances, sort_by_abundance=False
    )
...
def test_apply_threshold_and_format_empty_input(
    calculator_fixture: AbundanceCalculator,
):
    assert calculator_fixture.apply_threshold_and_format({}) == {}


def test_apply_threshold_and_format_exact_threshold_value(
    calculator_fixture: AbundanceCalculator,
):
    # calculator_fixture has threshold 0.01
    ...
    expected = {
        "StrainB": 0.99,
        "StrainA": 0.01,
    }  # StrainA is at threshold, should be included
    result = calculator_fixture.apply_threshold_and_format(
        relative_abundances, sort_by_abundance=True
    )
    assert result == expected
