...
def test_convert_assignments_custom_unassigned_marker(
    ...
    # Test correct assignments with custom marker
    correct_assignments: Dict[Union[str, int], Union[str, int]] = {
        "read1": 0,  # StrainA
        "read4": unassigned_marker_fixture,
    }
    expected_correct = {
    ...
    result_correct = calculator_fixture.convert_assignments_to_strain_names(
        correct_assignments, unassigned_marker=unassigned_marker_fixture
    )
...
def test_convert_assignments_default_unassigned_marker_na(
    ...
    with pytest.raises(
        ...
        calculator_fixture.convert_assignments_to_strain_names({"read_unknown_str": "UnknownStrainXYZ"})
...
def test_convert_assignments_invalid_marker_type(
    ...
    with pytest.raises(ValueError, match="unassigned_marker must be a non-empty string."):
        calculator_fixture.convert_assignments_to_strain_names({"r1": 0}, unassigned_marker="") # type: ignore
...
def test_calculate_raw_abundances_typical(
    ...
    assert raw_counts_with_unassigned == Counter(
        {"StrainA": 2, "StrainB": 1, unassigned_marker_fixture: 1}
    )
...
# test_convert_assignments_invalid_read_id_type removed as str(None) is "None", not empty.
